...
2016-07-12: Added date filters to subqueries to reduce complexity.
*/
...
LEFT outer Join
  (select 
   ...
   AND inpay.creation_date >= to_date('JAN-01-2015','MON-DD-YYYY')
   AND inpay.creation_date < SYSDATE + 30
   group by invoice_id
   ) p 
...
LEFT OUTER JOIN 
  (
   ...
   AND IDSTR.ACCOUNTING_DATE >= to_date('JAN-01-2015','MON-DD-YYYY')
   AND IDSTR.ACCOUNTING_DATE < SYSDATE + 30
...
LEFT OUTER JOIN 
  ...
  (  
    ...
    AND ah.hold_date >= to_date('JAN-01-2015','MON-DD-YYYY')
    AND ah.hold_date < SYSDATE + 30
...
(
  ...
  LEFT JOIN apps.gl_code_combinations gcc
    ON id.dist_code_combination_id = gcc.code_combination_id 
  LEFT JOIN apps.fnd_flex_values_vl f1
    ON gcc.segment2 = f1.flex_value
  where 1=1
    and f1.flex_value_set_id = 1013508
    AND ID.ACCOUNTING_DATE >= to_date('JAN-01-2015','MON-DD-YYYY')
    AND ID.ACCOUNTING_DATE < SYSDATE + 30
  group by invoice_id
) infra 
